...
def align_clinical_notes(base_dataset):
    """Extract text from NOTEEVENTS and store as 'text_note' attribute per visit."""
...
class ClinicalDecayModel(BaseModel):
    """
    RNN + optional PubMedBERT with time-decay risk adjustment.

    Args:
        dataset: PyHealth dataset object.
        use_llm (bool): Whether to include clinical notes via PubMedBERT.
        pos_weight (float): Weight for positive class in BCE loss.
        dropout_rate (float): Dropout probability before final classifier.
    """
    def __init__(self, dataset, use_llm: bool = True, pos_weight: float = 3.0, dropout_rate: float = 0.3):
        ...
        # LLM components (loaded even if use_llm=False for simplicity, but not used in forward)
    ...
    def forward(self, **kwargs):
        ...
        # LLM branch
        if self.use_llm:
            # Align text_input to batch size
            ...
            lm_output = self.lm(**tokens).last_hidden_state[:, 0, :]  # [CLS] token
        ...
        positive_decay = F.softplus(self.decay_rate)   # ensure positivity
...
def evaluate_with_threshold(model, dataloader, threshold: float = 0.5, device=None):
    """
    Evaluate model on a dataloader using a fixed classification threshold.
    Returns metrics and the predicted probabilities.
    """
    model.eval()
    all_probs = []
    all_labels = []
...
def main():
    ...
    # Compute theoretical pos_weight (just for info)
